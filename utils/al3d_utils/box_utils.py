...
def lidar_to_image(points, lidar_to_cam, cam_to_image, ignore=False):
    ...
    points = torch.cat(
        ...
        dim=1)
    points_rect = torch.matmul(
        points, torch.transpose(lidar_to_cam, 0, 1))

    image_projects = torch.matmul(
        points_rect[:, :3],
        torch.transpose(cam_to_image[:, :3], 0, 1))
    point_depth = image_projects[:, [2]]
    point_depth_abs = torch.abs(point_depth)
    image_projects = image_projects[:, :2]

    # For nuscenes multi-camera process
    # At least there are 4 corners in the image plane
    # And for calculating 3d-image-center won't do hard assignment,
    # wrong projected will be deleted by the condition of box2d.
    new_image_projects = []
    for i in range(0, point_depth.shape[0], 8):
        depth_obj = point_depth[i:i+8]
        depth_obj_abs = point_depth_abs[i:i+8]
        projs = copy.deepcopy(image_projects)[i:i+8]
        if (depth_obj > 0).sum().item() >= 4 or ignore:
            projs = projs / depth_obj_abs
        else:
            # projs = projs / depth_obj
            projs[:, :] = 0.
        # projs = projs / depth_obj_abs
        new_image_projects.append(projs)

    # empty new_image_projects can't be concatenated!
    if len(new_image_projects) == 0:
        return np.zeros((0, 2)), np.zeros((0, 1))

    new_image_projects = torch.cat(new_image_projects)

    if is_numpy:
        new_image_projects = new_image_projects.numpy()
        point_depth = point_depth.numpy()

    return new_image_projects, point_depth
