...
class DataProcessor(object):
    def __init__(self, processor_configs, point_cloud_range, training, num_point_features):
        ...
        self.voxel_coords_generator = None
    ...
    def generate_voxel_coords(self, data_dict=None, config=None):
        ...
        if self.voxel_coords_generator is None:
            self.voxel_coords_generator = VoxelGenerator(
        ...
        points = data_dict['points']
        _, tv_coordinates, _ = self.voxel_coords_generator.point_to_voxel(
            tv.from_numpy(points)
        )
        coordinates = tv_coordinates.numpy()

        data_dict['voxel_coords_downscale'] = coordinates
        return data_dict
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        points = data_dict['points']
        # update from MMDet repo, becasue of spconv2
    ...
    def pad_image(self, data_dict=None, config=None):
        ...
        max_w, max_h = config.TARGET_SHAPE
        if 'images' in data_dict:
            for cam in data_dict['images'].keys():
