...
class DSECTrainingDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, root_path, training=True, logger=None) -> None:
        ...
        self.gt_point_num = self.dataset_cfg.GT_POINT_NUM
    ...
    def init_infos(self):
        ...
        # for k in range(10):
            ...
            #     info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_fov_bbox_2pt_50m.pkl')
            # else:
            #     info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_fov_bbox_5pt_50m.pkl')
        ...
        self.logger.info('Total samples for DSEC dataset: %d' % (len(waymo_infos)))
        ...
        if self.dataset_cfg.SAMPLED_INTERVAL[self.mode] > 1:
            ...
            self.logger.info('Total sampled samples for DSEC dataset: %d' % len(self.infos))
