        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                if 'num_points_in_gt' in info:
                    ...
                    info['difficulty'] = np.zeros_like(info['num_points_in_gt'])
                    # if ( (info['num_points_in_gt'] > 1) & (info['num_points_in_gt'] < 5) ).any():
                    #     pdb.set_trace()
                        
                    zero_difficulty_mask = info['difficulty'] == 0
                    # pdb.set_trace()
                    # info['difficulty'][(info['num_points_in_gt'] > 1) & zero_difficulty_mask] = 1
                    # info['difficulty'][(info['num_points_in_gt'] <= 5) & zero_difficulty_mask] = 2
                    
                    # info['difficulty'][(info['num_points_in_gt'] >= 2) & zero_difficulty_mask] = 1
                    # info['difficulty'][(info['num_points_in_gt'] < 2) & zero_difficulty_mask] = 2
                    ...
                    # if len(info['difficulty'][(info['num_points_in_gt'] < 2) & zero_difficulty_mask]) > 0:
                    #     pdb.set_trace()
